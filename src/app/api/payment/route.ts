...
export const POST = async () => {
    ...
    try {
        ...
        formData.append("store_passwd", `${process.env.STORE_PASSWD}`);
        formData.append("total_amount", "1000");
        // formData.append("currency", "BDT");
        formData.append("tran_id", `${tran_id}`);
        formData.append("success_url", `${process.env.HOST_URL}/api/success?id=${tran_id}`);
        formData.append("fail_url", `${process.env.HOST_URL}/api/fail?id=${tran_id}`);
        formData.append("cancel_url", `${process.env.HOST_URL}/api/cancel?id=${tran_id}`);
        formData.append("ipn_url", `${process.env.HOST_URL}/api/ipn?id=${tran_id}`);
